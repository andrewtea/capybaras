...
export function showCapybaraEquipmentUI(player, capybara) {
...
function showEquipMenu(player, capybara, slot) {
    ...
    const equipForm = new ActionFormData()
        .title(`Equip ${slot}`)
        .button("Equip Head", head ? itemIcons[head.typeId] : "textures/blocks/barrier")
        .button("Unequip Head", "textures/ui/cancel")
        .divider()
        .button("Equip Body", body ? itemIcons[body.typeId] : "textures/blocks/barrier")
    ...
    equipForm.show(player).then((res) => {
        ...
        switch (res.selection) {
            case 0:
                if (!handItem) {
                    player.sendMessage("§cYou must hold something in your hand to equip!§c");
                    return;
                }
            ...
            case 2:
                if (!handItem) {
                    player.sendMessage("§cYou must hold something in your hand to equip!§r");
                    return;
                }
